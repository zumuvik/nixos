...
# as opposed to continuous adding of //@theme
...
if [[ -z "${NOTIFY_SEND}" ]]; then
    echo "Warning: Did not find 'notify-send', notifications won't work."
fi
...
# Indicate what entry is selected
...
Please update your config file if you have local modifications.</span>"""
        ...
        RES=$( create_theme_list | ${ROFI} ${THEME_FLAG} "${MORE_FLAGS[@]}" -cycle -selected-row "${SELECTED}" -mesg "${MESG}") || true
        RTR=$?
...
if select_theme && ((SELECTED > 0)); then
    # Apply the selected theme
    add_theme_to_config "${theme_names[${SELECTED}]}"

    # Send notification with the selected theme name
    local selection="${theme_names[${SELECTED}]}"
    if [[ -n "$NOTIFY_SEND" ]]; then
        notify-send -u low -i "${iDIR}/ja.png" "Rofi Theme applied:" "$selection" || true
    fi
fi
