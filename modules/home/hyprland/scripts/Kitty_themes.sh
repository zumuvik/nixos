...
apply_kitty_theme_to_config() {
  ...
  temp_kitty_config_file=$(mktemp) || return 1
  ...
  rm -f "$temp_kitty_config_file"

  for pid_kitty in $(pidof kitty 2>/dev/null || true); do
    if [[ -n "$pid_kitty" ]]; then
...
if [[ ! -f "$kitty_config" ]]; then
  notify_user "${iDIR}/error.png" "Error" "Kitty config not found at: $kitty_config"
  exit 1
fi

original_kitty_config_content_backup=$(cat "$kitty_config") || {
  notify_user "${iDIR}/error.png" "Error" "Failed to read kitty config"
  exit 1
}
