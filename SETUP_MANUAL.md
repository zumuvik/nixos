...
3. Подключитесь к интернету (если Wi-Fi, используйте `wpa_cli` или `nmtui`).

## 2. Разметка диска (Пример UEFI)

Если у вас обычный UEFI и один пустой диск (например, `/dev/nvme0n1`):
...
*(Если вы переустанавливаете существующий хост, например `nixlensk323` на то же самое железо, этот шаг можно пропустить!)*
